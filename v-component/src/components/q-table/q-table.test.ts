...
const data: IQTableOptions = {
  columns: [],
  dataSource: [],
  scroll: { x: 1500, y: 700 },
};
const render = () => {
  const { columns, dataSource, scroll } = data;
  return { columns, dataSource, scroll };
};
describe("测试render函数", () => {
  test("对render函数进行测试", () => {
...
let dom = new JSDOM(`<div>表格测试</div>`);
let messages: any = null;
dom.window.document
  .querySelector("div")
  ?.addEventListener("click", function (e) {
    return onSendMessage(e, data, "table");
  });
...
describe("测试dom点击事件传递Messge", () => {
  test("对clickFont dom点击事件进行测试", () => {
    ...
    expect(messages).toEqual({
      ...
      body: { columns: [], dataSource: [], scroll: { x: 1500, y: 700 } },
    });
  });
});
