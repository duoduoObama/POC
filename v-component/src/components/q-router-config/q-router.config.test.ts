import { IQRouterConfigOptions } from "../q-router-config/IQrouterConfig";
...
const data: IQRouterConfigOptions = { router: {} };
const render = () => {
  const { router } = data;
  return router;
};
...
const dom = new JSDOM(`<div>测试文本</div>`);
let messages: any = null;
dom.window.document
  .querySelector("div")
  ?.addEventListener("click", function (e) {
    return onSendMessage(e, data, "router-config");
  });
