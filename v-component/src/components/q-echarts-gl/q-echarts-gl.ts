...
export class QEchartsGl extends Component {
  ...
  render() {
    ...
    return html` <div id="chart-container">${text}</div> `;
  }
  ...
  async initCharts(): Promise<void> {
    ...
    this.myChart.on("datazoom", (params: any) => {
      const { batch = [] } = params;
      const [
        { startValue: startValue1, endValue: endValue1 },
        { startValue: startValue2, endValue: endValue2 },
      ] = batch;

      const changeData = this.data.slice().filter((item: any) => {
        if (
          (startValue1 < item[0] && endValue1 > item[0]) ||
          (startValue2 < item[1] && endValue2 > item[1])
        ) {
          return true;
        }
        return false;
