  ...
  createComponentInstance(root) {
    ...
    const component = {
      ...
      watch: {
        data: {
          handler(newValue) {
            this.options = _.cloneDeep(newValue.options);
          },
          deep: true,
        },
      },
      methods: {
        ...
        clickTitle(index, e, node) {
          ...
          const clickTab = root.querySelector("#tabs").children[index].children[0];
          function resetTabs() {
          ...
          resetTabs();
          root.querySelector(`#${this.options[index].id}`).style.display = "block";
          clickTab.id = "current";
        ...
        copyTabs(index) {
          ...
          nextTick(() => {
            ...
            this.reloadPageComponentRoot(elData, `#${contentDIV[index + 1].id}`);
          });
          return this.options;
        },
        deleteTabs(index) {
          ...
          function createChild(nodes, element = {}) {
            ...
            nodes.forEach(async (item, index) => {
              ...
              if (targetNode) {
                ...
                const refParent = element && element.id === parentId ? element : element.querySelector(`#${parentId}`);
            ...
            switch (item) {
              case "changeOptions":
                ...
                if (check(data.body[index])) {
                  ...
                  antd.message.error("(更改组件数据):接收数据与当前接收点不匹配!");
                }
                break;
              case "clickTitle":
                shchema = {
                ...
                if (check(data.body[index])) {
                  ...
                  antd.message.error("(切换tab标签):接收数据与当前接收点不匹配!");
                }
                break;
              case "addOptions":
                this.addOptions();
              ...
              case "copyTabs":
                ...
                if (check(data.body[index])) {
                  ...
                  antd.message.error("(复制tab标签):接收数据与当前接收点不匹配!");
                }
                break;
              case "deleteTabs":
                shchema = {
                ...
                if (check(data.body[index])) {
                  ...
                  antd.message.error("(删除tab标签):接收数据与当前接收点不匹配!");
                }
                break;
              case "addTabs":
                shchema = {
                ...
                if (check(data.body[index])) {
                  ...
                  antd.message.error("(在某个tab标签后增加标签):接收数据与当前接收点不匹配!");
                }
                break;
              default:
                break;
